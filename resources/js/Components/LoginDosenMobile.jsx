...
const defaultMenuAccount = {
    label: 'Akun Dosen',
    description: 'Akses khusus dosen untuk pengajuan dan monitoring PKM.',
    icon: 'fa-user-tie',
};
const defaultStatusSummaryCopy = {
    pkmFallbackDescription: 'Status kegiatan PKM aktif pada akun dosen.',
    diprosesDescription: 'Pengajuan PKM Anda sedang ditinjau oleh tim P3M.',
    ditangguhkanDescription: 'Dokumen perlu diperbarui sebelum diproses kembali.',
    ditolakDescription: 'Anda bisa menyiapkan pengajuan baru dari tab akses.',
    diterimaDescription: 'Pengajuan PKM Anda telah lolos dan siap masuk tahap berikutnya.',
    defaultDescription: 'Form pengajuan PKM siap diisi dari tab akses.',
};
const getStatusSummary = (submissionStatus, pkmStatusData = null, copy = defaultStatusSummaryCopy) => {
    if (submissionStatus === 'berlangsung' || submissionStatus === 'selesai') {
        return {
            eyebrow: 'Status PKM',
            label: submissionStatus === 'berlangsung' ? 'PKM Berlangsung' : 'PKM Selesai',
            icon: submissionStatus === 'berlangsung' ? 'fa-person-walking' : 'fa-flag-checkered',
            tone: submissionStatus === 'berlangsung' ? 'warning' : 'success',
            description: pkmStatusData
                ? `${pkmStatusData.nama} • ${pkmStatusData.desa}, ${pkmStatusData.kecamatan}`
                : copy.pkmFallbackDescription,
        };
    }
    ...
    switch (submissionStatus) {
        case 'diproses':
            return {
                ...
                icon: 'fa-clock',
                tone: 'info',
                description: copy.diprosesDescription,
            };
        case 'ditangguhkan':
            return {
                eyebrow: 'Status Pengajuan',
                ...
                icon: 'fa-file-pen',
                tone: 'warning',
                description: copy.ditangguhkanDescription,
            };
        case 'ditolak':
            return {
                eyebrow: 'Status Pengajuan',
                ...
                tone: 'danger',
                description: copy.ditolakDescription,
            };
        case 'diterima':
            return {
                eyebrow: 'Status Pengajuan',
                ...
                tone: 'success',
                description: copy.diterimaDescription,
            };
        default:
            return {
                eyebrow: 'Status Pengajuan',
...
function MobileStatusSummary({ statusInfo, compact = false }) {
    return (
        <div className={`login-dosen-mobile-status-card ${compact ? 'is-compact' : ''} tone-${statusInfo.tone}`}>
            <div className="login-dosen-mobile-status-icon">
                <i className={`fa-solid ${statusInfo.icon}`}></i>
            </div>
            <div className="login-dosen-mobile-status-copy">
                <span className="login-dosen-mobile-status-eyebrow">{statusInfo.eyebrow}</span>
    ...
    return (
        <>
            ...
            <aside className={`landing-mobile-drawer ${isOpen ? 'is-open' : ''}`}>
                ...
                <div className="login-dosen-mobile-menu-account">
                    <div className="login-dosen-mobile-menu-account-badge">
                        <span className="login-dosen-mobile-menu-account-icon">
                            <i className={`fa-solid ${menuAccount.icon}`}></i>
                        </span>
                        <div className="login-dosen-mobile-menu-account-copy">
                            <span>{menuAccount.label}</span>
                            <small>{menuAccount.description}</small>
                        </div>
                    </div>
                </div>

                <div className="login-dosen-mobile-menu-status">
...
function MobileAccessPanel({ statusInfo, renderAccessCard }) {
    return (
        <section className="landing-mobile-screen login-dosen-mobile-screen--access">
            <div className="login-dosen-mobile-access-shell">
                <MobileStatusSummary statusInfo={statusInfo} />
                {renderAccessCard()}
            </div>
        </section>
    );
...
export default function LoginDosenMobile({
    ...
    const statusInfo = useMemo(
        () => getStatusSummary(submissionStatus, pkmStatusData, statusSummaryCopy),
        [submissionStatus, pkmStatusData, statusSummaryCopy]
    );
    ...
    return (
        <div className="landing-mobile-app login-dosen-mobile-app">
            <MobileSplashScreen />
            <MobileMenuDrawer
                isOpen={isMenuOpen}
            ...
            {activeTab === 'akses' && (
                <MobileAccessPanel
                    statusInfo={statusInfo}
                    renderAccessCard={accessCardRenderer}
                />
            )}

            <nav className="landing-mobile-tabbar" aria-label={navigationLabel}>
                {[
