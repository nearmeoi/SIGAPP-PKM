...
const Dashboard: React.FC<any> = ({ stats, recentPengajuan, monthlyStats = Array(12).fill(0), recentLogs = [] }) => {
    ...
    const maxStat = Math.max(...monthlyStats, 10); // Minimum 10 scale
        ...
        <AdminLayout title="">
                ...
                <div className="divide-y divide-zinc-100">
                    {(recentPengajuan || []).length === 0 ? (
                        ...
                        (recentPengajuan || []).map((p: any) => {
                            ...
                            return (
                                <div key={p.id_pengajuan} className="flex items-center justify-between px-6 py-4 hover:bg-zinc-50/50 transition-colors">
                                    {/* Left: Nama - Kegiatan */}
                                    <div className="min-w-0 flex-1 pr-6 flex items-center">
                                        <div className="text-[14px] font-semibold text-zinc-900 truncate block">
                                            {p.user?.name}
                                            <span className="text-zinc-400 font-normal mx-2">—</span>
                                            <a href={`/admin/pengajuan/${p.id_pengajuan}`} className="hover:text-indigo-600 transition-colors">
                                                {p.judul_kegiatan}
                                            </a>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-end gap-6 flex-shrink-0">
                                        <div className={`flex items-center justify-center gap-2 px-3 py-1 rounded-md text-[12px] font-medium ${st.bg} ${st.text} whitespace-nowrap w-[100px]`}>
                                            <span className={`w-1.5 h-1.5 rounded-full ${st.dot}`}></span>
                                            {st.label}
                                        </div>
                                        <div className="text-[12px] font-medium text-zinc-400 whitespace-nowrap text-right w-[110px]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div className="lg:col-span-2 bg-white rounded-xl border border-zinc-200 shadow-sm p-6 relative overflow-hidden flex flex-col">
                    ...
                    <div className="relative w-full flex-1 flex flex-col min-h-[180px]">
                        ...
                        <div className="relative z-10 flex h-full items-end gap-2 pl-8 pt-2 pb-[1px]">
                            {monthlyStats.map((val: number, i: number) => {
                                ...
                                return (
                                    <div key={i} className="flex-1 flex flex-col justify-end items-center group relative h-full">
                                        ...
                                        <div
                                            className="w-full bg-zinc-900 rounded-t-sm opacity-20 group-hover:opacity-60 transition-all cursor-pointer"
                ...
                <div className="bg-white rounded-xl border border-zinc-200 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-zinc-200/80 bg-zinc-50/50">
                        ...
                        <p className="text-[12px] text-zinc-500 mt-0.5">Catatan sistem admin</p>
                    </div>
                            ...
                            recentLogs.map((log: any, i: number) => {
                                ...
                                return (
                                    <div key={i} className="flex items-start gap-4">
                                        <div className={`w-2.5 h-2.5 rounded-full mt-1.5 border-2 flex-shrink-0 border-white ring-1 ring-zinc-200 ${sCfg.dot}`} />
                                        <div>
