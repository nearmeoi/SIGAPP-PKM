...
Route::middleware('auth')->group(function () {
    ...
    // Global search API — returns recent items when q is empty, search results when q is provided
    Route::get('/api/search', function (Request $request) {
        $q = trim($request->input('q', ''));
        $limit = 5;

        $formatPengajuan = fn ($p) => ['id' => $p->id_pengajuan, 'title' => $p->judul_kegiatan, 'subtitle' => ucfirst($p->status_pengajuan), 'url' => '/admin/pengajuan/'.$p->id_pengajuan];
        $formatUser = fn ($u) => ['id' => $u->id_user, 'title' => $u->name, 'subtitle' => $u->email.' ('.$u->role.')', 'url' => '/admin/users'];
        $formatPegawai = fn ($p) => ['id' => $p->id_pegawai, 'title' => $p->nama_pegawai, 'subtitle' => 'NIP: '.($p->nip ?? '-'), 'url' => '/admin/pegawai'];
        $formatAktivitas = fn ($a) => ['id' => $a->id_aktivitas, 'title' => $a->pengajuan->judul_kegiatan ?? 'Aktivitas #'.$a->id_aktivitas, 'subtitle' => ucfirst($a->status_pelaksanaan), 'url' => '/admin/aktivitas/'.$a->id_aktivitas];
        $formatTestimoni = fn ($t) => ['id' => $t->id_testimoni, 'title' => $t->nama_pemberi, 'subtitle' => 'Rating: '.$t->rating.'/5', 'url' => '/admin/testimoni'];
        $formatArsip = fn ($a) => ['id' => $a->id_arsip, 'title' => $a->nama_dokumen, 'subtitle' => $a->jenis_arsip, 'url' => '/admin/arsip'];

        if (strlen($q) < 2) {
            // Return recent items (latest 5 from each table)
            return response()->json([
                'pengajuan' => Pengajuan::latest()->limit($limit)->get(['id_pengajuan', 'judul_kegiatan', 'status_pengajuan'])->map($formatPengajuan),
                'users' => User::latest()->limit($limit)->get(['id_user', 'name', 'email', 'role'])->map($formatUser),
                'pegawai' => Pegawai::latest()->limit($limit)->get(['id_pegawai', 'nama_pegawai', 'nip'])->map($formatPegawai),
                'aktivitas' => Aktivitas::with('pengajuan')->latest()->limit($limit)->get(['id_aktivitas', 'id_pengajuan', 'status_pelaksanaan'])->map($formatAktivitas),
                'testimoni' => App\Models\Testimoni::latest()->limit($limit)->get(['id_testimoni', 'nama_pemberi', 'rating'])->map($formatTestimoni),
                'arsip' => Arsip::latest()->limit($limit)->get(['id_arsip', 'nama_dokumen', 'jenis_arsip'])->map($formatArsip),
            ]);
        }
        ...
        return response()->json([
            'pengajuan' => Pengajuan::where('judul_kegiatan', 'like', $like)->limit($limit)->get(['id_pengajuan', 'judul_kegiatan', 'status_pengajuan'])->map($formatPengajuan),
            'users' => User::where('name', 'like', $like)->orWhere('email', 'like', $like)->limit($limit)->get(['id_user', 'name', 'email', 'role'])->map($formatUser),
            'pegawai' => Pegawai::where('nama_pegawai', 'like', $like)->orWhere('nip', 'like', $like)->limit($limit)->get(['id_pegawai', 'nama_pegawai', 'nip'])->map($formatPegawai),
            'aktivitas' => Aktivitas::with('pengajuan')->where('status_pelaksanaan', 'like', $like)->limit($limit)->get(['id_aktivitas', 'id_pengajuan', 'status_pelaksanaan'])->map($formatAktivitas),
            'testimoni' => App\Models\Testimoni::where('nama_pemberi', 'like', $like)->orWhere('pesan_ulasan', 'like', $like)->limit($limit)->get(['id_testimoni', 'nama_pemberi', 'rating'])->map($formatTestimoni),
            'arsip' => Arsip::where('nama_dokumen', 'like', $like)->limit($limit)->get(['id_arsip', 'nama_dokumen', 'jenis_arsip'])->map($formatArsip),
        ]);
    })->name('api.search');
